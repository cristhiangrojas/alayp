...
class Study extends CI_Controller 
{
	function __construct()
	{
		...
		$this->content_centro 	= 'dashboard/modulos/menu/study/panel_centro/panel_centro';
		$this->footer 			= 'dashboard/footer';

		$this->content_agregar_panel_centro 	= 'dashboard/modulos/menu/study/panel_centro/agregar_panel_centro';
		$this->content_visualizar_panel_centro 	= 'dashboard/modulos/menu/study/panel_centro/visualizar_panel_centro';
		$this->content_editar_panel_centro 		= 'dashboard/modulos/menu/study/panel_centro/editar_panel_centro';
		/*FIN VIEW PANEL CENTRO*/

		/*VIEW PANEL DERECHO*/
		$this->header 			= 'dashboard/header';
		$this->content_derecha 	= 'dashboard/modulos/menu/study/panel_derecha/panel_derecha';
		$this->footer 			= 'dashboard/footer';
	}
	...
	function panel_izquierdo()
	{
		if($this->session->userdata('is_logued_in') == true)
		{
			...
			$data['listado_newsfeed'] = $this->study_ml->listado_newsfeed();
	...
	function panel_centro()
	{
		if($this->session->userdata('is_logued_in') == true)
		{
			...
			$data['listado_study'] = $this->study_ml->listado_study();
	...
	function procesar_agregar_panel_centro()
	{
		if($this->session->userdata('is_logued_in') == true)
		{
		    ...
		    {
			    $config['upload_path'] 		= 'uploads/study/panel_centro/';
	        	...
	        	if (!$this->upload->do_upload()) 
			    {
					...
					redirect('administrador/modulos/menu/study/study/panel_centro', 'refresh');
					...
					$agregar_panel_centro = $this->study_ml->agregar_panel_centro($data);
	...
	function visualizar_panel_centro()
	{
		if($this->session->userdata('is_logued_in') == true)
		{
			...
			$id_study_panel_centro 		= $this->input->get('id_study_panel_centro');

			$row = $this->study_ml->get_panel_centro($id_study_panel_centro);
	...
	function procesar_edit_panel_centro()
	{
		if($this->session->userdata('is_logued_in') == true)
		{
			$id_study_panel_centro 	= $this->input->post('id_study_panel_centro');
		    $titulo 				= $this->input->post('titulo');
		    $link 					= $this->input->post('link');
	        $descripcion 			= $this->input->post('descripcion');
			$f_registro 			= date('m-d-Y H:i:s');
		    ...
		    if (empty($_FILES['userfile']['name']))
			{
				$data = array
				(
					'id_study_panel_centro'	=> $id_study_panel_centro,
					'fecha_hora'			=> $f_registro,
					'titulo'				=> $titulo,
					'link'					=> $link,
					'descripcion'			=> $descripcion
				...
				$editar_panel_centro = $this->study_ml->editar_panel_centro($data);
	...
	function eliminar_panel_centro()
	{
		if($this->session->userdata('is_logued_in') == true)
		{
			...
			$eliminar_panel_centro = $this->study_ml->eliminar_panel_centro($id_study_panel_centro);
	...
	function procesar_sponsors()
	{
		if($this->session->userdata('is_logued_in') == true)
		{
			$config['upload_path'] 		= 'uploads/study/panel_derecha/sponsors/';
        	$config['allowed_types'] 	= 'jpg|jpeg|png|JPG|JPEG|PNG';
        	$config['remove_spaces']	= TRUE;
        	$config['max_size']    		= '1024';
		    ...
		    {
		    	$this->session->set_flashdata('error_imagen','No se ha subido imagen');
				redirect('administrador/modulos/menu/study/study/panel_derecha', 'refresh');
				...
				$data = array
				(
					'id_study_panel_derecha_sponsors'	=> '1',
					'imagen'							=> $archivo
				);

				$edit_panel_derecha_sponsors = $this->study_ml->edit_panel_derecha_sponsors($data);
